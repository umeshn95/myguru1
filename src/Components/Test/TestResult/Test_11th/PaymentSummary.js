...
const PaymentSummary = () => {
  ...
  const getData = () => {
    ...
    setLoading(true);
    // fetch("https://myguruonline.herokuapp.com${process.env.REACT_APP_API_URL}/api/studentquestion", {
              ...
              <div className="col-md-12">
                <div className="full">
                  <div style={{ background: "#770202" }} className="w-100">
                    <h1 className="text-center py-3 text-light">
                      Payment Summary
                    </h1>
                  </div>
                  <div className="full mt-3 back_payment">
                    ...
                    {data.map((e, i) => (
                      <div className="float-left" key={i}>
                        {data.length === 0 ? (
                          ...
                          <>
                            <h1 className="text-start mt-3 fw-bold text-light">
                              Number of questions : <span>{count}</span>
                            </h1>
                            <h1 className="text-start mt-3 fw-bold text-light">
                              Duration : <span>{e.duration}</span> minutes
                            </h1>
                            <h1 className="text-start mt-3 fw-bold text-light">
                              Student Name :{" "}
                            ...
                            <h1 className="text-start mt-3 fw-bold text-light">
                              Class : <span>{e.grade.grade}</span>
                            </h1>
                            <h1 className="text-start mt-3 fw-bold text-light">
                              Test Instruction :{" "}
                              <span>{e.grade.testInstruction}</span>
                            </h1>
                          </>
                        )}
                      </div>
                    ...
                    <div className="float-right mr-5">
                      ...
                      />
                      {data.map((e, i) => (
                        <h1 className="mt-4 mr-5 text-light" key={i}>
                          <b>Amount to be paid: INR {e.amount}</b>
                        </h1>
                      ))}
                      {data.length === 0 ? (
                        <>
                          <Link>
                            <button
                              disabled
                              className="pay_btn float-right mr-5 disabled"
                            >
                              Pay
                            </button>
                          </Link>
                        ...
                        <>
                          <Link to="/paymentconfirm">
                            <button className="pay_btn float-right mr-5">
                              Pay
                            </button>
                          </Link>
                        </>
